...
const state = JSON.parse(localStorage.getItem('neon-last-call-flat') || 'null') || {money:120,rep:0,night:1,served:0,guestIndex:0,relations:{},upgrades:{steady:false,ice:false,scanner:false}};
let currentGuest, currentRecipe, mix;
let pourTimer=null;
let audioEnabled=true;
const $=id=>document.getElementById(id);

function save(){localStorage.setItem('neon-last-call-flat',JSON.stringify(state));}
...
function renderBottles(){const rack=$('bottleRack');rack.innerHTML='';Object.entries(INGREDIENTS).forEach(([key,it])=>{const card=document.createElement('div');card.className='bottle-card';card.style.setProperty('--bottle',it.color);card.innerHTML=`<div class="bottle-art"><div class="bottle-shape"></div></div><b>${it.name}</b><small>${it.abv?it.abv+'% Vol.':'alkoholfrei'}</small><button class="pour-btn">Einschenken</button>`;const btn=card.querySelector('button');const start=e=>{e.preventDefault();beginPour(key)};btn.addEventListener('pointerdown',start);btn.addEventListener('pointerup',stopPour);btn.addEventListener('pointerleave',stopPour);btn.addEventListener('pointercancel',stopPour);rack.appendChild(card);});}
function beginPour(key){stopPour();const step=state.upgrades.steady?2:3;$('pourStream').style.setProperty('--stream',INGREDIENTS[key].color);$('pourStream').style.height='145px';pourTimer=setInterval(()=>addIngredient(key,step),90);beep(260,.08)}
function stopPour(){if(pourTimer){clearInterval(pourTimer);pourTimer=null}$('pourStream').style.height='0'}
