...
const GUESTS = [
 {id:'nyx',name:'Nyx Vale',short:'Nyx',role:'Netzläuferin',img:'guest_nyx.svg',hue:0,recipe:'mirage',trust:10,chem:8,mood:'wachsam',scenes:[
  ...
  {text:'Im Helix-Netz gibt es einen blinden Fleck von genau sieben Minuten. Würdest du hineingehen, ohne zu wissen, wer ihn geöffnet hat?',choices:[['Nur mit einem Rückweg und dir am anderen Ende.',4,2,''],['Sieben Minuten reichen für sehr viel Ärger.',2,1,''],['Wenn du mitkommst, riskiere ich es.',0,4,'flirt']]},
  {text:'Eine letzte Sache: Wenn wir dort Erinnerungen finden, die jemand gelöscht haben wollte – gehören sie dann den Menschen, aus denen sie stammen?',choices:[['Erinnerungen gehören niemandem außer ihrem Ursprung.',5,0,''],['Manche Wahrheiten brauchen erst einen sicheren Ort.',3,1,''],['Frag mich nach dem Drink noch einmal.',0,2,'flirt']]}]},
 {id:'aria',name:'Aria Kade',short:'Aria',role:'Konzernkurierin',img:'guest_aria.svg',hue:0,recipe:'ghost',trust:6,chem:3,mood:'angespannt',scenes:[
  ...
  {text:'Der Behälter hatte innen Kratzspuren. Nicht zufällig – jemand hat eine Nachricht eingeritzt: „Mira hört uns.“ Was bedeutet das?',choices:[['Miras Implantat empfängt mehr als Musik.',4,0,''],['Das Paket wusste, wer draußen ist.',3,1,''],['Wir sollten Mira sofort warnen.',5,0,'']]},
  {text:'Wenn Helix mich findet, werden sie behaupten, ich hätte den Auftrag verraten. Glaubst du einer Kurierin, die ihr Paket verloren hat?',choices:[['Ich glaube der Person, die zurückgekommen ist.',5,1,''],['Vertrauen entsteht aus dem nächsten Schritt.',3,0,''],['Ich glaube dir mehr, als mir lieb ist.',1,4,'flirt']]}]},
 {id:'vex',name:'Vex Rainer',short:'Vex',role:'Straßenrennfahrerin',img:'guest_vex.svg',hue:0,recipe:'eclipse',trust:8,chem:7,mood:'aufgedreht',scenes:[
  ...
  {text:'Mein Bruder fuhr früher für Helix. Nach seinem letzten Rennen kannte er mich nicht mehr. Meinst du, sie haben seine Erinnerungen überschrieben?',choices:[['Dann holen wir sie zurück, wenn sie noch existieren.',5,0,''],['Sol sollte seine alten Akten prüfen.',3,0,''],['Er hat dich vielleicht vergessen – du ihn aber nicht.',2,2,'']]},
  {text:'Für den Coup brauche ich jemanden auf dem Beifahrersitz. Ruhige Hände, schlechte Impulskontrolle. Kennst du da jemanden?',choices:[['Ich kenne die Strecke und halte mich fest.',3,2,''],['Nyx navigiert, ich sichere den Rückweg.',4,0,''],['Du beschreibst gerade mein bestes Profil.',0,5,'flirt']]}]},
 {id:'sol',name:'Sol Mercer',short:'Sol',role:'Ex-Cop',img:'guest_sol.svg',hue:0,recipe:'blackout',trust:4,chem:2,mood:'misstrauisch',scenes:[
  ...
  {text:'Sorn war mein Informant. In seiner letzten Nachricht sagte er: „Die Toten arbeiten nachts.“ Damals hielt ich es für Angst.',choices:[['Vielleicht meinte er synthetische Kopien.',4,0,''],['Oder Menschen ohne ihre eigenen Erinnerungen.',5,0,''],['Du hättest ihm zuhören sollen.',-1,0,'']]},
  {text:'Wenn wir das Labor öffnen, kann ich Helix öffentlich machen – oder die Daten verkaufen und verschwinden. Was würdest du wählen?',choices:[['Veröffentlichen. Sonst beginnt alles von vorn.',5,0,''],['Erst die Gefangenen retten, dann entscheiden.',5,1,''],['Verschwinden klingt besser, wenn man nicht allein geht.',0,3,'flirt']]}]},
 {id:'mira',name:'Mira Quell',short:'Mira',role:'Holo-Sängerin',img:'guest_mira.svg',hue:0,recipe:'mirage',trust:7,chem:10,mood:'melancholisch',scenes:[
  ...
  {text:'Die zweite Stimme kennt ein Lied, das meine Mutter nur einmal für mich gesungen hat. Helix kann diese Erinnerung unmöglich besitzen.',choices:[['Vielleicht stammt die Stimme aus deiner eigenen Kopie.',4,1,''],['Dann hat Helix schon länger Zugriff auf dich.',3,0,''],['Das Lied gehört weiterhin dir.',4,2,'']]},
  {text:'Wenn ich auf der Bühne zusammenbreche, spielst du den Song zu Ende oder ziehst du den Stecker?',choices:[['Ich ziehe dich von der Bühne. Kein Signal ist wichtiger.',5,1,''],['Nyx trennt nur die fremde Verbindung.',4,0,''],['Ich fange dich auf, bevor du fällst.',1,5,'flirt']]}]},
 {id:'kade',name:'Kade Zero',short:'Kade',role:'Helix-Deserteur',img:'guest_kade.svg',hue:0,recipe:'ghost',trust:2,chem:1,mood:'erschöpft',scenes:[
  ...
  {text:'Meine früheste Erinnerung ist ein weißer Raum und eine Stimme, die mich Kade Nummer Null nennt. Was, wenn es nie ein Davor gab?',choices:[['Dann beginnt dein Leben mit dem ersten eigenen Entschluss.',5,0,''],['Eine Herkunft ist keine Identität.',4,1,''],['Du bist real genug, um Angst davor zu haben.',3,1,'']]},
  {text:'Im Kern des Labors liegt eine Abschaltroutine für alle synthetischen Systeme. Auch für mich. Würdest du sie trotzdem auslösen?',choices:[['Nur du entscheidest über dein Leben.',5,1,''],['Wir finden einen anderen Weg.',4,0,''],['Ich lasse dich nicht dort zurück.',1,4,'flirt']]}]}
];
...
function beep(freq=440,dur=.05){const C=window.AudioContext||window.webkitAudioContext;if(!C)return;beep.ctx=beep.ctx||new C();const o=beep.ctx.createOscillator(),g=beep.ctx.createGain();o.frequency.value=freq;o.type='sine';g.gain.setValueAtTime(.025,beep.ctx.currentTime);g.gain.exponentialRampToValueAtTime(.001,beep.ctx.currentTime+dur);o.connect(g);g.connect(beep.ctx.destination);o.start();o.stop(beep.ctx.currentTime+dur);}
...
function openModal(type){$('modal').classList.remove('hidden');if(type==='contacts'){ $('modalTitle').textContent='Kontakte';$('modalContent').innerHTML='<div class="contact-grid">'+GUESTS.map(g=>{const r=state.relations[g.id]||{trust:g.trust,chem:g.chem};return `<div class="contact"><img src="${g.img}" alt="${g.name}"><div><b>${g.name}</b><small>${g.role}</small><p>Vertrauen ${r.trust} · Chemie ${r.chem}</p></div></div>`}).join('')+'</div>';}else{$('modalTitle').textContent='Schwarzmarkt';const items=[['steady','Präzisionsausgießer','Langsameres, genaueres Einschenken',90],['ice','Kryo-Eisfach','Mehr und klarere Eiswürfel',75],['scanner','Geschmacksscanner','+4 Punkte auf jede Wertung',140]];$('modalContent').innerHTML='<div class="shop-grid">'+items.map(([id,n,d,p])=>`<div class="shop-item"><b>${n}</b><small>${d}</small><p>${state.upgrades[id]?'Installiert':p+' €'}</p><button ${state.upgrades[id]?'disabled':''} data-buy="${id}" data-price="${p}">${state.upgrades[id]?'Installiert':'Kaufen'}</button></div>`).join('')+'</div>';document.querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>buy(b.dataset.buy,+b.dataset.price));}}
